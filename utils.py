...
def get_stat(img, access_token):
    ...
    if response:
        ...
        result = [str(i['words']).replace('·', '').replace(':', '')
                  for i in result]
        ...
        artifact.vice_stat0_value = str(
            result[vice_stat_index[0]].split('+')[1])
        artifact.vice_stat1 = result[vice_stat_index[1]].split('+')[0]
        artifact.vice_stat1_value = str(
            result[vice_stat_index[1]].split('+')[1])
        artifact.vice_stat2 = result[vice_stat_index[2]].split('+')[0]
        artifact.vice_stat2_value = str(
            result[vice_stat_index[2]].split('+')[1])
        if len(vice_stat_index) == 4:
            artifact.vice_stat3 = result[vice_stat_index[3]].split('+')[0]
            artifact.vice_stat3_value = str(
                result[vice_stat_index[3]].split('+')[1])
